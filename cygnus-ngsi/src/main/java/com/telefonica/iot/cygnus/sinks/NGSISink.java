...
public abstract class NGSISink extends CygnusSink implements Configurable {
    ...
    private Status processNewBatches() {
        ...
        try {
            ...
            LOGGER.info("Rollback transaction by ChannelException  (" + ex.getMessage() + ")  Sink: " + this.getName());
            txn.rollback();
        } catch (Exception ex) {
            LOGGER.info("Rollback transaction by Exception  (" + ex.getMessage() + ")  Sink: " + this.getName());
            txn.rollback();
        } finally {
            txn.close();
        }
