...
public class NGSIMongoSink extends NGSIMongoBaseSink {
    ...
    private void persistAggregation(NGSIGenericAggregator aggregator) throws CygnusPersistenceError {
        ...
        for (int i = 0 ; i < jsonObjects.size() ; i++) {
            ...
            JsonElement attType;
            if (rowAttrPersistence) {
                JsonElement el = aggregator.getAggregation().get(NGSIConstants.ATTR_TYPE);
                if (el != null) {
                    attType = el.get(i);
                    if ( (attType != null &&
                ...
                for (String key : basicDBObject.keySet()) {
                    JsonElement el = aggregator.getAggregation().get(key + NGSIConstants.AUTOGENERATED_ATTR_TYPE);
                    if (el != null) {
                        attType = el.get(i);
                        if ( (attType != null &&
                              attType.isJsonPrimitive()) &&
