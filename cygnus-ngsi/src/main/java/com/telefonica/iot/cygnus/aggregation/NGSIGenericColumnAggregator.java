...
public class NGSIGenericColumnAggregator extends NGSIGenericAggregator {
    ...
    public void aggregate(NGSIEvent event) {
        ...
        if (numPreviousValues > 0 && isEnableLastData()) {
            if ((CommonUtils.getTimeInstantFromString(getLastData().get(getLastDataTimestampKey()).get(0).getAsString())).longValue() < recvTimeTs) {
                isLastData = true;
        ...
        if ((numPreviousValues == 0 || isLastData) && isEnableLastData() ) {
            lastData.put(NGSIConstants.RECV_TIME_TS+"C", new ArrayList<JsonElement>());
            lastData.put(NGSIConstants.RECV_TIME, new ArrayList<JsonElement>());
            lastData.put(NGSIConstants.FIWARE_SERVICE_PATH, new ArrayList<JsonElement>());
            lastData.put(NGSIConstants.ENTITY_ID, new ArrayList<JsonElement>());
            lastData.put(NGSIConstants.ENTITY_TYPE, new ArrayList<JsonElement>());
            lastData.get(NGSIConstants.RECV_TIME_TS+"C").add(new JsonPrimitive(Long.toString(recvTimeTs)));
            lastData.get(NGSIConstants.RECV_TIME).add(new JsonPrimitive(recvTime));
            lastData.get(NGSIConstants.FIWARE_SERVICE_PATH).add(new JsonPrimitive(getServicePathForData()));
            lastData.get(NGSIConstants.ENTITY_ID).add(new JsonPrimitive(entityId));
            lastData.get(NGSIConstants.ENTITY_TYPE).add(new JsonPrimitive(entityType));
        }
        for (NotifyContextRequest.ContextAttribute contextAttribute : contextAttributes) {
            String attrName = contextAttribute.getName();
            ...
            if ((numPreviousValues == 0 || isLastData) && isEnableLastData() ) {
                if (lastData.containsKey(attrName)) {
                    lastData.get(attrName).set(0, attrValue);
                    lastData.get(attrName + "_md").set(0, jsonAttrMetadata);
                    lastData.get(attrName + NGSIConstants.AUTOGENERATED_ATTR_TYPE).set(0, new JsonPrimitive(attrType));
                } else {
                    ArrayList<JsonElement> valuesLastData = new ArrayList<JsonElement>();
                    valuesLastData.add(attrValue);
                    lastData.put(attrName, valuesLastData);
                    ArrayList<JsonElement> valuesMdLastData = new ArrayList<JsonElement>();
                    valuesMdLastData.add(jsonAttrMetadata);
                    lastData.put(attrName + "_md", valuesMdLastData);
                    ArrayList<JsonElement> valuesTypeLastData = new ArrayList<JsonElement>();
                    valuesTypeLastData.add(new JsonPrimitive(attrType));
                    lastData.put(attrName + NGSIConstants.AUTOGENERATED_ATTR_TYPE, valuesTypeLastData);
                }
            }
        } // for
