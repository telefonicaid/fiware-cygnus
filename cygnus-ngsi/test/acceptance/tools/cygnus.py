        ...
        for i in range(int(self.instances_number)):
            ...
            for i in range(len(sinks_list)):
                if sinks_list[i].find(HDFS_SINK)>=0:
                    ...
                    ops_list = cygnus_agent.config_mongo_sink(sink=sinks_list[i], channel=self.__get_channels(sinks_list[i]), host_port="%s:%s" % (world.config['mongo']['mongo_host'], world.config['mongo']['mongo_port']), user=world.config['mongo']['mongo_user'], password=world.config['mongo']['mongo_password'], auth_source=world.config['mongo']['auth_source'], replicaset=world.config['mongo']['replicaset'])
