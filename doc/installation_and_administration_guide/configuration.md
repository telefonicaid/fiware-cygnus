...
```
{
    "grouping_rules": [
        {
	# An unique unsigned integer-based identifier.
            "id": 1,
	# List of fields that will be cocatenated for regular expression matching.
	# The order of these fields is important: "entityID", "entityType" and "servicePath".
            "fields": [
                ...
            ],
	# Java-like regular expression to be applied on the concatenated fields. 
	# Special characters like '\' must be escaped ('\' is escaped as "\\\\").
            "regex": "...",
	# Name of HDFS file or CKAN resource where the data will be effectively persisted. 
	# In the case of MySQL, Mongo and STH this sufixes the table/collection name.
            "destination": "...",
	# New "fiware-servicePath" replacing the notified one. The sinks will translate 
	# this into the name of the HDFS folder or CKAN package where the above 
	# destination entity will be placed.
	# In the case of MySQL, Mongo and STH this prefixes the table/collection name.
            "fiware_service_path": "..."
        },
        ...
    ]
