...
public class ArcgisFeatureTableTest implements ArcgisBaseTest{
	...
	private ArcgisFeatureTable connectArcgis(){
		return new ArcgisFeatureTable(
				ArcgisBaseTest.getFeatureUrl(), 
	...
	public void arcgisAddUpdateBatchTest() {
		...
		try{
			...
			for (Feature feature_2 : list) {
				Integer id = (Integer) feature_2.getAttributes().get(uniqueField);
				switch (id){
				case 9991:
				case 9993:
					assertTrue (feature_2.getAttributes().get("descripcion").equals("modificado"));
					break;
				case 9992:
					assertTrue (feature_2.getAttributes().get("descripcion").equals("Prueba 2 Arcgis.java"));
					break;
				default:
