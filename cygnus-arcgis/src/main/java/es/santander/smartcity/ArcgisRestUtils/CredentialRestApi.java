...
public abstract class CredentialRestApi extends RestApi {
	...
	protected void setCredential(Credential credential) throws ArcgisException{
		if (false && credential != null && tokenGenUrl != null && credential.isExpired()){
			//TODO: ¿¿refrescar token solo cuando se hace un getCredential??
			this.credential = RestAuthentication.createToken(credential, tokenGenUrl, referer);
		}else{
			this.credential = credential;
		}
	...
	/**
	 * @param tokenGenUrl the tokenGenUrl to set
	 */
