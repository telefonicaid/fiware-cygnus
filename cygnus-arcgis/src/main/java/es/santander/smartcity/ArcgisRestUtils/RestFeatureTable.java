...
public class RestFeatureTable extends CredentialRestApi {
	...
	public RestFeatureTable (String url, Credential credential, String tokenGenUrl, BaseLoggerInterface parentLogger) throws ArcgisException{
...
//	}
	...
	/**
	 * @param tableAttributes the tableAttributes to set
	 */
	...
	/**
	 * @param uniqueAttributes the uniqueAttributes to set
	 */
	...
	public ResultPage<Feature> getFeatureList(String whereClause, int pageOffset, String token) throws ArcgisException {
		...
		String responseJSON = null;
	      ...
	      if (!fullUrl.endsWith("/query")) fullUrl += "/query";
	...
	public void sendFeatureList(List<Feature> featureList, String action) throws ArcgisException{
		...
		String fullUrl = serviceUrl.toString();
		if (!fullUrl.endsWith("/" + action)) fullUrl += "/" + action;
	...
	public void deleteEntities(List<String> objectIdList) throws ArcgisException{
		...
		String fullUrl = serviceUrl.toString();
		if (!fullUrl.endsWith("/deleteFeatures")) fullUrl += "/deleteFeatures";
	...
	protected void getTableAttributesInfo() throws ArcgisException{
		...
		try{
		      ...
		      if (response.isSuccessful()){
		    	  ...
		    	  String errorMsg = "getTableAttributesInfo: Unexpected server response, Error: " + response.getErrorCode() + "\n" + response.getErrorMessage();
		    	  errorMsg += " \n\t token: " + token;
		    	  logError(errorMsg);
		    	  throw new ArcgisException(errorMsg);
		      }
	...
	protected void getAttributeIndexFromJson (String jsonStr) throws ArcgisException{
		...
		try{
			...
			throw new ArcgisException("Error setting unique attributes, " + e.getClass().getSimpleName() + "  " + e.getMessage());
		}
	}
	 ...
	 * @throws ArcgisException
	 */
    protected ResultPage<Feature> resultPageFromJson(String responseJson, String listTag, int pageOffset) throws ArcgisException {

    	boolean hasMore = false;
    	
    	if ("".equals(listTag)) listTag = "features";
    	List<Feature> featureList = new ArrayList<Feature>();
