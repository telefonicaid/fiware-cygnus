...
# Delete the Maven home directory.
directory "#{node[:maven][:path]}/apache-maven-#{node[:maven][:version]}" do
	recursive true
	action :delete
	only_if { ::File.exists?("#{node[:maven][:path]}/apache-maven-#{node[:maven][:version]}") } 
end
