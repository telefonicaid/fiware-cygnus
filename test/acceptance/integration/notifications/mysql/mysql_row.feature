...
Feature: Store in mysql new notifications per row from context broker
    ...
    Scenario Outline: stored new notifications in mysql from context broker with or without metadata
       Given cygnus is installed with type "row"
         ...
         And a tenant "tenant_3", service path "default", resource "default", with attribute number "4", attribute name "random" and attribute type "celcius"
        When receives a notification with attributes value "random", metadata value "True" and content "<content>"
        Then Validate that the attribute value, metadata and type are stored in mysql
    ...
    Scenario Outline: stored new notifications in mysql from context broker with or without metadata
       Given cygnus is installed with type "row"
         And "mysql" is installed correctly
         And a tenant "tenant_3", service path "default", resource "default", with attribute number "<attributes_number>", attribute name "random" and attribute type "celcius"
   ...
   Scenario Outline:  store in mysql new notifications with different service path values
       Given cygnus is installed with type "row"
         And "mysql" is installed correctly
         And a tenant "<tenant>", service path "<service_path>", resource "default", with attribute number "2", attribute name "random" and attribute type "celcius"
        When receives a notification with attributes value "random", metadata value "True" and content "<content>"
        Then Validate that the attribute value, metadata and type are stored in mysql
         And Close mysql connection
    ...
    Scenario Outline: stored new notifications in ckan with different matching_table patterns
       Given cygnus is installed with type "row"
        ...
        Then changes new destination "<new_destination>" where to verify in table "<new_service_path>"
         And Validate that the attribute value, metadata and type are stored in mysql
         And Close mysql connection
